...
class BOMModel extends Model
{
    ...
    public function get_warehousesbyId($warehouse_id)
    {
        $query = "SELECT warehouse_id,name FROM warehouses where warehouse_id=".$warehouse_id;
    ...
    public function update_mrp_scheduling($bom_id, $planning_id)
    {
        ...
        if ($_POST["operation_type"] == 'operation_adding_area') {
            $array_op = array();
            $data_op = !empty($_POST["operation_row"]) ? $_POST["operation_row"] : [];
        }
