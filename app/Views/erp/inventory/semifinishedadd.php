...
<div class="alldiv">
    <?php
    echo form_open(url_to('erp.inventory.semifinishedadd'), array(
        "class" => "flex",
        "id" => "semifinished_add_form"
    ));
    ?>
    ...
    <div class="form-width-2">
        <div class="form-group  field-ajax" data-ajax-url="<?php echo url_to('erp.inventory.ajaxsemifinishedcodeunique') . '?'; ?>">
            <label class="form-label">Code</label>
            <input type="text" name="code" class="form_control field-check" value="<?= "SEM-".str_pad(intval($semiFinished_count) + 1,4,0,STR_PAD_LEFT) ?>" />
            <p class="error-text"></p>
        </div>
    </div>
    <div class="form-width-2">
    ...
    <div class="form-width-1">
        <div class="form-group textRight">
            ...
            <button class="btn bg-primary" type="button" id="semifinished_add_submit">Save</button>
        </div>
    </div>
    </form>
</div>
...
<script type="text/javascript">
    ...
    let form = document.getElementById("semifinished_add_form");
    let validator = new FormValidate(form);

    let lock = false;
    document.getElementById("semifinished_add_submit").onclick = function(evt) {
        if (!lock) {
            lock = true;
            validator.validate(
                (params) => {
