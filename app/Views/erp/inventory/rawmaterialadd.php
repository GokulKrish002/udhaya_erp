<div class="alldiv flex widget_title">
    <h3>Create Raw Materials</h3>
    <div class="title_right">
        <a href="<?php echo url_to('erp.inventory.rawmaterials'); ?>" class="btn bg-success"><i class="fa fa-reply"></i> Back </a>
    </div>
</div>
<div class="alldiv">
    <?php
    echo form_open(url_to('erp.inventory.rawmaterialadd'), array(
        "class" => "flex",
    ...
    <div class="form-width-2">
        <div class="form-group  field-ajax" data-ajax-url="<?php echo url_to('erp.inventory.rawmaterialcodeunique') . '?'; ?>">
            <label class="form-label">Code</label>
            <input type="text" name="code" class="form_control field-check" value="<?= "RAW-".str_pad(intval($rawMaterial_count) + 1,4,0,STR_PAD_LEFT) ?>" />
            <p class="error-text"></p>
        </div>
    </div>
    <div class="form-width-2">
    ...
    <div class="form-width-1">
        <div class="form-group textRight">
            ...
            <button class="btn bg-primary" type="button" id="rawmaterial_add_submit">Save</button>
        </div>
    </div>
    </form>
</div>
...
<script type="text/javascript">
    ...
    let form = document.getElementById("rawmaterial_add_form");
    let validator = new FormValidate(form);

    let lock = false;
    document.getElementById("rawmaterial_add_submit").onclick = function(evt) {
        if (!lock) {
            lock = true;
            validator.validate(
                (params) => {
